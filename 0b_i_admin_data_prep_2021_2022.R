for (year in c(2021, 2022)){
    ...
    participants_work_history = work_history %>%
      mutate(
        ...
        BEGINN = pmax(ymd(BEGINN), ymd("2020-09-30")),
      ...
      filter(ENDE > ymd("2020-09-30")) %>% #  restrict data to past 10 years
      filter(ENDE < ymd(ifelse(year == 2021, 
        "2021-03-01", "2022-12-01"))) %>% #  restrict data to prior intervention start for group 2, in year 2021
      mutate(duration = ENDE - BEGINN) %>%
      group_by(PSTNR, EMPLOYED) %>%
      summarise(days = as.integer(sum(duration)), spells = n()) 
