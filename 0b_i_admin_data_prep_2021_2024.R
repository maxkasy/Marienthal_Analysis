# Define paths for each year in a tidy way
for (year in c(2021, 2022, 2023, 2024)) {
  ...
  if (year == 2021) {
    ...
    data_path <- paste0(veracrypt_path, "jobguarantee/2023-02-admin-data-raw/")
    survey_path <- paste0(veracrypt_path, "jobguarantee/2023-02-survey-data-processed/")
    admin_out <- paste0(veracrypt_path, "jobguarantee/2023-02-admin-data-processed/")
    enddate <- "2023-02-28"      
  } else if (year == 2024) {
...
} # comment this } out to run the entire script as a loop and process all years of data
    ...
    # file_path = paste0(data_path, "Leistungsbezug_ProgrammteilnehmerInnen.csv")
    benefits =
      read_delim(file_paths[[4]], delim=";",
                 locale = locale(encoding = "latin1", decimal_mark = ","))
    ...
    participants_benefits <- benefits %>%
      mutate(
        PSTNR = as.integer(PSTNR),
        STICHTAG = as.Date(STICHTAG),  # Convert datetime to date
        TAGSATZLEIST = as.integer(TAGSATZLEIST)
      ) %>%
      group_by(PSTNR) %>%
      arrange(STICHTAG) %>%
      slice(n()) %>%  # Keep most recent record per participant
      ungroup() %>%
      select(PSTNR, STICHTAG, TAGSATZLEIST) %>%
      rename(benefit_level_post_intervention = TAGSATZLEIST)
    
    
    ### 2. work history (days EMPLOYED since program start) ---------------------------------
    work_history =
      read_delim(file_paths[[5]], delim=";",
                 locale = locale(encoding = "latin1", decimal_mark = ",")) %>%
      rename(PSTNR = PSTKEY) # rename to PSTNR for merging
    
  
    # Check for problematic date values
    work_history %>%
      filter(is.na(as.Date(BEGINN, format = "%Y-%m-%d")) | 
               is.na(as.Date(ENDE, format = "%Y-%m-%d"))) %>%
      count()
    ...
    participants_work_history <- work_history %>%
      mutate(
        # Convert STATUS to EMPLOYED flag (1 = employed, 0 = not employed)
        EMPLOYED = as.integer(!(STATUS %in% c("AL", "D2", "SC", "LS", "SF", "SR"))),
        
        # Ensure dates are properly formatted and handle potential parsing issues
        BEGINN = pmax(as.Date(BEGINN, format = "%Y-%m-%d"), 
                      as.Date("2020-09-30")),
      ...
      filter(
        ENDE > as.Date("2020-09-30"),  # Restrict data to past 10 years
        ENDE < ymd(enddate)  # Use dynamic enddate
 #       ENDE < as.Date(ifelse(year == 2021, "2021-03-01", "2022-12-01"))  # Restrict to prior intervention start
      ) %>%
      # Calculate duration and summarize
      mutate(
        duration = as.integer(ENDE - BEGINN)
      ) %>%
      group_by(PSTNR, EMPLOYED) %>%
      summarise(
        days = sum(duration, na.rm = TRUE),
        spells = n(),
        .groups = "drop"  # Explicitly drop grouping structure
      )
    
    # reshape to get average days in unemployment AND employment
    participants_work_history <- participants_work_history %>% 
      mutate(EMPLOYED = as.character(EMPLOYED),
             EMPLOYED = replace(EMPLOYED, EMPLOYED == "0", "unemployed"),
             EMPLOYED = replace(EMPLOYED, EMPLOYED == "1", "employed")
      )
    
    participants_work_history <- 
      pivot_wider(data = participants_work_history, 
                  id_cols = PSTNR, 
                  names_from = EMPLOYED, 
                  values_from = c("days", "spells"))
    
    # replace NA with 0
    participants_work_history <- participants_work_history %>% 
      mutate(days_unemployed_sv = replace_na(days_unemployed, 0),
             days_employed_sv = replace_na(days_employed, 0),
             spells_unemployed_sv = replace_na(spells_unemployed, 0),
             spells_employed_sv = replace_na(spells_employed, 0) 
      ) %>% 
      select(-days_unemployed, -days_employed, -spells_unemployed, -spells_employed)
    ...
    work_history_ams <- work_history_ams %>% 
      ...
      work_history_ams <- work_history_ams %>% 
        mutate( 
          TAGE_BESCHAEFTIGUNG_GESAMT = `TAGE_BESCHAEFTIGUNG_GESAMT_2023-02-28` - `TAGE_BESCHAEFTIGUNG_GESAMT_2020-09-30` ,
          TAGE_BESCHAEFTIGUNG_GEF = `TAGE_BESCHAEFTIGUNG_GEF_2023-02-28` - `TAGE_BESCHAEFTIGUNG_GEF_2020-09-30` ,
          TAGE_BESCHAEFTIGUNG_UNGEF = `TAGE_BESCHAEFTIGUNG_UNGEF_2023-02-28` - `TAGE_BESCHAEFTIGUNG_UNGEF_2020-09-30` , 
          TAGE_BESCHAEFTIGUNG_SELBST = `TAGE_BESCHAEFTIGUNG_SELBST_2023-02-28` - `TAGE_BESCHAEFTIGUNG_SELBST_2020-09-30` ,
          TAGE_AMS_VORMERKUNG = `TAGE_AMS_VORMERKUNG_2023-02-28` - `TAGE_AMS_VORMERKUNG_2020-09-30` , 
          TAGE_OUT_OF_LABOUR_FORCE = `TAGE_OUT_OF_LABOUR_FORCE_2023-02-28` - `TAGE_OUT_OF_LABOUR_FORCE_2020-09-30`
        )
    } else if (year == 2024) {
      work_history_ams <- work_history_ams %>% 
        mutate( 
