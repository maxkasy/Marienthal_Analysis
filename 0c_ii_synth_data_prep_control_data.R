...
data_path = paste0(veracrypt_path, "jobguarantee/2020-09-municipal-data-raw/") 
...
months_list <- list("2019-12", "2020-07")
...
sex_AL_wide_tmp = data_sex_AL_tmp %>%
  ...
  mutate(month = months_list[[y]] %>% # add year or months for merging
         as.character()) %>% # change variable type for merging
    rename(men_AL = M,
           women_AL = W)
...
### migration ####
file_path[[y]] = paste(data_path,  path_list[[y]], files_used[6], sep = "")
names(file_path)[y] <- names_list[[y]]
...
Ubenefit_AL = read_delim(
  ...
  mutate(month = "2019-07") %>% # add year or month for merging
...
files_used_pop = c(
  ...
  "BEVOELKERUNG_NOE_AUSBILDUNG.dsv",
...
# Change GKZ to double for merging
...
file_path = paste(file_paths_pop[3], sep = "")
...
pop_status_annual <- data_pop %>%
  group_by(GKZ, GEMEINDE, month, status) %>%
...
data_wage_POP = read_excel(file_paths_wage, 1)
...
municipalities =
  ...
  group_by(GKZ, month) %>%
  left_join(mean_LZBL_AL, by = c("GKZ", "month")) %>% # long & short term UE longitudinal
  ...
  left_join(edu_POP_wide, by = c("GKZ", "month")) %>%  # edu POP cross-section 
  ...
  left_join(data_tax_long_sep, by = c("GKZ", "month")) # communal tax
